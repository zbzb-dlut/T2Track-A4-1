...
class UAVTRACK(BaseTracker):
    ...
    def initialize(self, image, info: dict):
        ...
        with torch.no_grad():
            self.template_feat,_ = self.network.forward_encoder(z=self.template_list[-1])
    ...
    def track(self, image, info: dict = None):
        ...
        # run the encoder
        with torch.no_grad():
            _,search_feat = self.network.forward_encoder(x=search_list[-1])

        with torch.no_grad():
            enc_opt = self.network.forward_neck(template_feats=self.template_feat,
                                      search_feats=search_feat)
        # run the decoder
        with torch.no_grad():
        ...
        if 0: #self.debug == 1:
            ...
            score = conf_score.item()  # 自动处理 .cpu() 和 .detach()，适用于标量 tensor
            score_text = f'Score: {score:.3f}'  # 格式化为三位小数
            # 4. 在图像上添加文字（例如显示在左上角或框上方）
            ...
            # 设置文字位置（例如：放在边界框上方）
            text_x = int(x1)
            text_y = int(y1 - 10)  # 比框高一点
            # 如果文字位置太靠上，调整到图像内部
            if text_y < 10:
                text_y = int(y1 + h + 20)  # 放在框下方
            # 绘制文字
            cv2.putText(image_BGR, score_text, (text_x, text_y), font, font_scale, color, thickness, cv2.LINE_AA)
