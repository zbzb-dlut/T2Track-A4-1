...
def calc_seq_err_robust(pred_bb, anno_bb, dataset, target_visible=None):
    ...
    if torch.isnan(anno_bb).any():
        if dataset == 'uav123' or dataset == 'uavtrack' or dataset == 'uav123_10fps' or dataset == 'uav123' or dataset == 'uavtrack112' or dataset == 'uav123_l' :
            pass
        else:
...
def extract_results(trackers, dataset, report_name, skip_missing_seq=False, plot_bin_gap=0.05,
    ...
    for seq_id, seq in enumerate(tqdm(dataset)):
        ...
        for trk_id, trk in enumerate(trackers):
            # Load results
            if seq.dataset in ['trackingnet', 'got10k', 'lasot', 'lasot_extension_subset', 'otb', 'uavdt', 'visdrone','uavtrack112', 'dtb', 'uav123_10fps', 'uav123', 'nfs', 'tnl2k','uav123_l','webuav3m']:
